...
df['target_res'] = df['load'] - df['load'].shift(-24)
...
predictions = model.predict(X_test) + df['lag_24']
naive_pred = X_test['lag_24']
logger.info('trained baseline linear regression')

#save results
pred_series = pd.DataFrame({'date': df.loc[y_test.index, 'date'], 
                            'predicted_load': predictions, 'naive_pred': naive_pred})
